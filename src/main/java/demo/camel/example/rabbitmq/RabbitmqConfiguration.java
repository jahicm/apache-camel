package demo.camel.example.rabbitmq;
...
@Configuration
@ConditionalOnExpression("${demo.camel.example.rabbitmq.enabled=true}")
@Slf4j
public class RabbitmqConfiguration {
	...
	public static final String RABBIT_URI = "spring-rabbitmq:" + EXCHANGE_WEATHER_DATA + "?" + "queues=%s&"
			+ "routingKey=%s&" + "arg.queue.autoDelete=false&" + "autoDeclare=true&" + "concurrentConsumers=20&"
	...
	/**
	 * camel-spring-rabbitmq lib will either create all channels under one
	 * connection or one channel per connection using Scope=PROTOTYPE will create
	 * new connection per route
	 *
	 * @return
	 */
	@Bean
